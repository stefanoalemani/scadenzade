...
import view.palette as palette_ui
...
class MainApp:
    ...
    """
    def __init__(self, dati_clienti, dati_fornitori, dati_table_head, run=True):
        # richiama "view" per creare user interface
        self.main = view_ui.MainView(dati_clienti, dati_fornitori, dati_table_head, self)
        self.loop = urwid.MainLoop(self.main, palette_ui.palette, unhandled_input=self.exit_on_q)
        if run:
...
def main():
    ...
    if dati:
        MainApp(dati["dati_clienti"], dati["dati_fornitori"], dati["dati_table_head"])
    else:
