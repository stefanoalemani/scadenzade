...
import models.readwrite_csv_xml as scadenze
import glob
import os
...
class ScadDati:
    ...
    def read_xml(self, manual_scad=False, data_manual=None):
        ...
        for file_path in glob.glob(self.xml_filename):
            scad_temp = scadenze.read_xml_scad(file_path, manual_scad, data_manual)
            self.scad_all.append(scad_temp)
        return self.scad_all
    
    def xml_to_csv(self, filename):
        '''
        make a file in CSV format starting by self.scadAll (invoice expiration) 
        with an header (headFile) usefully to make a scheduler, and 
        body (content) data. Arg "fileName" is the name of the file that will be
        generated.
        '''
        # Intestazione file
        ...
        for scad in self.scad_all:
            ...
            for i in range(num_scad):
                content.append([
                    ...
                    scad[3]["Numero"],
                    scad[3]["Data"],
                    f"{float(scad[4]['ImponibileImporto']) + float(scad[4]['Imposta']):.2f}",
                    scad[1][f"DataScadenzaPagamento{i+1}"],
                    f"{float(scad[1][f'ImportoPagamento{i+1}']):.2f}",
                    scad[1][f"ModalitaPagamento{i+1}"],
                    scad[5]["FDenominazione"],
                    scad[5]["FIdCodice"]
                ])

        content.sort(key=lambda x: (x[6], x[4]))
        scadenze.write_csv_clifor(filename, header, content)
        print(f"File '{filename}' formato CSV creato con successo!")
...
class ViewScadGen:
    ...
    def scad_cli_for(self, year):
        ...
        # (data_limite)
        # data_limite = datetime.strptime("2025-07-31", "%Y-%m-%d")
        # dati_filtrati = [riga for riga in dati if datetime.strptime(riga["Data
        # ScadenzaPagamento"], "%Y-%m-%d") == data_limite]
        ########################################################################

        # read file
        list_reader = scadenze.read_csv_raw(self.csv_file)
        totale_mese = []
        mese_filtro = 1
        totale_mese = []
        for mese in range(1, 13):
            dati_filtrati = [
                ...
                if datetime.strptime(riga["DataScadenzaPagamento"], "%Y-%m-%d").month == mese
                and datetime.strptime(riga["DataScadenzaPagamento"], "%Y-%m-%d").year == year
            ]
            totale = sum(float(r["ImportoPagamento"]) for r in dati_filtrati)
            totale_mese.append(f"{totale:.2f}")
