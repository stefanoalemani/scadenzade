...
from config.constants import YEAR_ACT
# anno di riferimento
...
class ToolBar(urwid.WidgetWrap):
    ...
    def handle_data_path(self):
        ...
        def on_path_selected(new_path):
            def chiudi_popup(loop, user_data):
                pop.close_popup()
            # Salva il nuovo percorso nel file
            success = self.controller.set_start_paths({
                "fornitori": [new_path[0]],
                "clienti": [new_path[1]]
            })
            if success:
                self.start_path = new_path
                evento = control.widget_control.Event("text-statusbar", self, {
            ...
            self.controller.emetti_evento(evento)
            self.app.loop.set_alarm_in(1.0, chiudi_popup)
    ...
    def handle_elaborate_invoice(self):
        ...
        def on_elaborazione_completata(confermato, esito):
            def chiudi_popup(loop, user_data):
                pop.close_popup()

            tipo = "Info" if esito else "Warning"
            messaggio = (
                "Elaborazione annullata dall'utente" if not confermato else
                "Elaborazione terminata con successo" if esito else
                "Elaborazione non riuscita"
            )
            evento = control.widget_control.Event("text-statusbar", self, {tipo: messaggio})
            self.controller.emetti_evento(evento)
            self.app.loop.set_alarm_in(1.0, chiudi_popup)

        contab = MakeInvoiceCsv(start_path=start_path, on_close=on_elaborazione_completata)
...
class SelectYears(urwid.WidgetWrap):
    def __init__(self, years, on_close=None):
...
class MakeInvoiceCsv(urwid.WidgetWrap):
    ...
    def confirm(self, button):
        if self.on_close:
            esito = self.make_csv()
            self.on_close(True, esito)  # True = utente ha confermato
